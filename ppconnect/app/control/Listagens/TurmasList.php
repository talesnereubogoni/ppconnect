<?php
/**
 * TurmasList Listing - Cadastro de turmas
 * @author  Tales Nereu Bogoni
 */
class TurmasList extends TPage
{
    ...
    public function __construct()
    {
        ...
        $this->setActiveRecord('Turmas');   // defines the active record
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_search_Turmas');
        ...
        //$this->form->addActionLink(_t('New'), new TAction(['TurmasForm', 'onEdit']), 'fa:plus green');
        ...
        $column_nome = new TDataGridColumn('nome', 'Identificação', 'left');
        $column_cursos_id = new TDataGridColumn('cursos->nome', 'Curso', 'left');
        ...
        $column_cursos_id->setAction(new TAction([$this, 'onReload']), ['order' => 'cursos_id']);
        
        $action1 = new TDataGridAction(['TurmasForm', 'onEdit'], ['id'=>'{id}']);
        ...
        $action_bt_novo = new TAction(['TurmasForm', 'onEdit'], ['id' => ''] );
    ...
    **/
     public static function verificaIntegridade($object){
        ...
        $retorno = false;
        $count= ProfessoresDaDisciplina::where('turmas_id','=',$object->id)->count();
        if($count==0){
             $count=Provas::where('turmas_id','=',$object->id)->count();
             if($count==0){
                 $count=TurmasDoPolo::where('turmas_id','=',$object->id)->count();
                 if($count==0){
                     $retorno = true;
                 }
