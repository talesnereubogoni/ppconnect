...
class SystemUser extends TRecord
{
   ...
   /**
    ...
    public function delete($id = NULL)
    {
        ...
        $count = ProfessoresDaDisciplina::where('professor_id', '=', $this->id)->count();
        if($count==0)
            $count += Questoes::where('professor_id', '=', $this->id)->count();
        if($count>0){
            new TMessage('error', "O professor não pode ser excluído.");
        } else {
           parent::delete($id);
           $this->clearParts();         
        } 
     ...
     */
    public function set_polo(Turmas $object)
    {
        $this->polo = $object;
     ...
     */
    public function get_polo()
    {
        // loads the associated object
        if (empty($this->polos_id))
            $this->polo = new Polos($this->polos_id);
