    ...
    public static function onLogin($param)
    {
        ...
        try
        {
            ...
            if ($user)
            {
                ...
                try{
                    $identificacao = new Identificacao(1);
                    TSession::setValue('conf_codigo', $identificacao->codigo);
                    TSession::setValue('conf_palavra_passe', $identificacao->palavra_passe);
                    TSession::setValue('conf_path_bd', $identificacao->path_bd);
                    $aux = TurmasDoPolo::where('tutor_id', '=', $user->id)->load();
                    if($aux!=null)
                        TSession::setValue('conf_turma', $aux[0]->turmas_id);
                    else
                        TSession::setValue('conf_turma', null);
                } catch (Exception $e) {}
            }
            TTransaction::close();
        }
